            ...
            <div className='d-flex flex-row justify-content-between'>
                <div className='row'>
                    ...
                    </div>
                    {
                        ...
                        <div className={`row pl-5 ${barClick && 'w-100 text-left'}`}>
                            <div className={`px-4 py-4 ${barClick && 'w-100'}`} style={{borderBottom:'1px solid #900000'}}  >
                                <MyLink to={`${match.url}`} onClick={() => { setBarClick(false) }} text="Home" currentLink={location.pathname + location.search} />
                            </div>
                            <div className={`px-4 py-4 ${barClick && 'w-100'}`} style={{borderBottom:'1px solid #900000'}} >
                                <MyLink to={`${match.url}${route.burger}`} onClick={() => { setBarClick(false) }} text="Burgers" currentLink={location.pathname + location.search} />
                            </div>
                            <div className={`px-4 py-4 ${barClick && 'w-100'}`} style={{borderBottom:'1px solid #900000'}} >
                ...
                </div>
                {
                    ...
                    <div className={`d-flex`} style={{ fontSize: 25 }}>
                        <div className=" px-3 pt-3" style={{ cursor: 'pointer' }}><span style={{ color: Colors.white }}><i className="fab fa-facebook" /></span></div>
                        <div className=" px-3 pt-3" style={{ cursor: 'pointer' }}><span style={{ color: Colors.white }}><i className="fab fa-instagram" /></span></div>
                        <div className=" px-3 pt-3" style={{ cursor: 'pointer' }}><span style={{ color: Colors.white }}><i className="fas fa-bicycle"></i></span></div>
                    </div>
                }
                {
                    // color: '#FF6C33'
